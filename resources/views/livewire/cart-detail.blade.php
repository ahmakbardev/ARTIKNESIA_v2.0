<div>
        ...
        @if($art_count > 0)
            @foreach($art_items as $index => $item)
                <div class="grid grid-cols-4 gap-x-3">
                    <img src="{{ $item['image'] }}" class="rounded-lg object-cover col-auto" alt=""/>
                    <div class="col-span-3 text-start flex flex-col justify-between">
                        <h6 class="text-2xl font-semibold line-clamp-1">{{ $item['name'] }}</h6>
                        <div class="flex flex-row justify-between items-end">
                            <div>
                                <p class="text-lg">{{ $item['quantity'] }} Item</p>
                                <p class="text-3xl font-semibold w-fit">${{ $item['price'] }}</p>
                            </div>
    ...
    <div class="w-full flex justify-end">
        <button class="bg-primary px-5 py-3 text-2xl font-bold text-white rounded-xl">Checkout Sekarang</button>
    </div>
</div>
