...
@section('content')
    @php
        $shareIconImages = [
            ...
            'images/icons/instagram-icon.svg',
            'images/icons/facebook-icon.svg',
    ...
    <div class="flex flex-col gap-y-3 relative max-w-[768px] mx-auto pt-10">
        ...
        <div class="hidden xl:block absolute right-[calc(100%+1rem)] w-[235px] px-2 text-sm">
            ...
            <div class="flex items-center gap-2 border-b-2 border-[#D0D5DD] pb-6 mb-6 px-2">
                <img class="w-11 rounded-full object-contain" src="{{ asset('images/profile/default.png') }}"
                    alt="">
            ...
            <div class="flex flex-row flex-wrap gap-x-1.5 gap-y-1 mb-3">
                @foreach ($article->getTags() as $item)
                    <span
                        class="text-white text-sm font-semibold bg-primary rounded-[0.625rem] px-2 py-1">#{{ $item }}</span>
                @endforeach
            ...
            <div class="flex flex-row flex-wrap gap-x-1.5 gap-y-1">
                ...
                @foreach ($categories as $item)
                    <span
                        class="text-white text-sm font-semibold bg-primary rounded-[0.625rem] px-2 py-1">{{ $item->name }}</span>
                @endforeach
            </div>
        </div>

        {{-- Main Article --}}
        <div class="truncate max-w-64 text-sm font-poppins text-black/70"><a href="/" class="hover:underline">Home</a>
            > <a href="{{ route('article.index') }}" class="hover:underline">Artikel</a> > {{ $article->title }}</div>
